...
pub use crate::network::behaviour::Gossipsub;
...
impl<AppReqId: ReqId, TSpec: EthSpec> Network<AppReqId, TSpec> {
    pub async fn new(ctx: ServiceContext<'_>) -> error::Result<(Self, Arc<NetworkGlobals<TSpec>>)> {
        let mut config = ctx.config.clone();
        info!("Libp2p Service starting");
        ...
        let network_globals = {
            ...
            let meta_data = utils::load_or_build_metadata(&config.network_dir);
        ...
        let eth2_rpc = RPC::new(ctx.fork_context.clone());

        let discovery = {
            // Build and start the discovery sub-behaviour
            let mut discovery =
                Discovery::new(&local_keypair, &config, network_globals.clone()).await?;
    ...
    async fn start(&mut self, config: &crate::NetworkConfig) -> error::Result<()> {
        ...
        info!(peer_id = %enr.peer_id(), bandwidth_config = format!("{}-{}", config.network_load, NetworkLoad::from(config.network_load).name), "Libp2p Starting");
        ...
        debug!(address = ?config.listen_address, tcp_port = config.libp2p_port, udp_port = discovery_string, "Attempting to open listening ports");
        ...
        let mut dial = |mut multiaddr: Multiaddr| {
            ...
            match self.swarm.dial(multiaddr.clone()) {
                Ok(()) => debug!(%multiaddr, "Dialing libp2p peer"),
                Err(err) => {
        ...
        for topic_kind in &config.topics {
            if self.subscribe_kind(topic_kind.clone()) {
                ...
                warn!(%topic_kind, "Could not subscribe to topic");
            }
        }

        if !subscribed_topics.is_empty() {
            info!(?subscribed_topics, "Subscribed to topics");
        }
    ...
    pub fn report_message_validation_result(
        ...
        if let Err(e) = self.gossipsub_mut().report_message_validation_result(
            ...
            warn!(message_id = %message_id, peer_id = %propagation_source, error = ?e, "Failed to report message validation");
        }
    }
    ...
    fn dial_cached_enrs_in_subnet(&mut self, subnet: Subnet) {
        let predicate = subnet_predicate::<TSpec>(vec![subnet]);
        ...
        while let Poll::Ready(Some(swarm_event)) = self.swarm.poll_next_unpin(cx) {
            let maybe_event = match swarm_event {
                ...
                SwarmEvent::IncomingConnection {
                    ...
                    trace!(%local_addr, from = %send_back_addr, "Incoming connection");
                    None
                }
                SwarmEvent::IncomingConnectionError {
                    ...
                    debug!(%local_addr, from = %send_back_addr, %error, "Failed incoming connection");
                    None
                }
                ...
                SwarmEvent::BannedPeer {
                    ...
                    debug!(%peer_id, "Banned peer connection rejected");
                    None
                }
                ...
                SwarmEvent::ListenerClosed {
                    ...
                    error!(?addresses, ?reason, "Listener closed");
