...
pub fn load_or_build_metadata<E: EthSpec>(network_dir: &std::path::Path) -> MetaData<E> {
            ...
            match MetaDataV2::<E>::from_ssz_bytes(&metadata_ssz) {
                ...
                Err(_) => {
                    match MetaDataV1::<E>::from_ssz_bytes(&metadata_ssz) {
                        ...
                        Err(e) => {
                            debug!(error = ?e, "Metadata from file could not be decoded");
                        }
                    }
                }
            }
...
pub(crate) fn save_metadata_to_disk<E: EthSpec>(dir: &Path, metadata: MetaData<E>) {
        ...
        .and_then(|mut f| f.write_all(&metadata.as_ssz_bytes()))
    {
        ...
        Err(e) => {
            warn!(file = format!("{:?}{:?}", dir, METADATA_FILENAME), error = %e, "Could not write metadata to disk");
        }
    }
}
