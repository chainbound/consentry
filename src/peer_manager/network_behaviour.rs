...
impl<TSpec: EthSpec> NetworkBehaviour for PeerManager<TSpec> {
    ...
    fn poll(
        ...
        loop {
            match self.inbound_ping_peers.poll_next_unpin(cx) {
                ...
                Poll::Ready(Some(Err(e))) => {
                    error!(error = %e, "Failed to check for inbound peers to ping")
                }
        ...
        loop {
            match self.outbound_ping_peers.poll_next_unpin(cx) {
                ...
                Poll::Ready(Some(Err(e))) => {
                    error!(error = %e, "Failed to check for outbound peers to ping");
                }
                Poll::Ready(None) | Poll::Pending => break,
            }
        }
        ...
        if !matches!(
            ...
            loop {
                match self.status_peers.poll_next_unpin(cx) {
                    ...
                    Poll::Ready(Some(Err(e))) => {
                        error!(error = %e, "Failed to check for peers to ping")
                    }
                    Poll::Ready(None) | Poll::Pending => break,
                }
            }
    ...
    fn inject_connection_established(
        ...
        debug!(%peer_id, connection = ?endpoint.to_endpoint(), "Connection established");
        ...
        match self.ban_status(peer_id) {
            ...
            BanResult::BadScore => {
                ...
                error!(%peer_id, "Connected to a banned peer, re-banning");
                // Reban the peer
            ...
            BanResult::BannedIp(ip_addr) => {
                ...
                debug!(%peer_id, banned_ip = %ip_addr, "Peer connected via banned IP. Banning");
