...
impl<TSpec: EthSpec> Discovery<TSpec> {
    ...
    pub fn add_enr(&mut self, enr: Enr) {
        ...
        if let Err(e) = self.discv5.add_enr(enr) {
            debug!(error = e, "Could not add peer to the local routing table");
        }
    }
    ...
    pub fn update_eth2_enr(&mut self, enr_fork_id: EnrForkId) {
        ...
        let _ = self
            ...
            .map_err(|e| warn!(error = ?e, "Could not update eth2 ENR field"));
    ...
    fn start_subnet_query(&mut self, subnet_queries: Vec<SubnetQuery>) {
        ...
        if !filtered_subnet_queries.is_empty() {
            ...
            let subnet_predicate = subnet_predicate::<TSpec>(filtered_subnets);
    ...
    fn start_query(
        ...
        if !contains_queries {
            debug!("No subnets included in this request. Skipping discovery request.");
            return;
        }
        ...
        let enr_fork_id = match self.local_enr().eth2() {
            Ok(v) => v,
            Err(e) => {
                error!(error = e, "Local ENR has no fork id");
                return;
            }
        };
                ...
                match query.result {
                    ...
                    Ok(r) => {
                        ...
                        queries.iter().for_each(|query| {
                            ...
                            let subnet_predicate = subnet_predicate::<TSpec>(vec![query.subnet]);
                    ...
                    Err(e) => {
                        warn!(error = %e, ?subnets_searched_for, "Grouped subnet discovery query failed");
                    }
                }
            }
        }
