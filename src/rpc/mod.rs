...
impl<Id: ReqId, TSpec: EthSpec> RPC<Id, TSpec> {
    pub fn new(fork_context: Arc<ForkContext>) -> Self {
            ...
            match self.limiter.allows(&peer_id, req) {
                ...
                Err(RateLimitedErr::TooLarge) => {
                    ...
                    if matches!(protocol, Protocol::BlocksByRange) {
                        debug!(?req, "Blocks by range request will never be processed");
                    } else {
                ...
                Err(RateLimitedErr::TooSoon(wait_time)) => {
                    debug!(?peer_id, ?req, ?wait_time, "Request exceeds the rate limit");
                    // send an error code to the peer.
                    // the handler upon receiving the error code will send it back to the behaviour
                    self.send_response(
                        peer_id,
